...
```{r make_surrogs_CP_hays, echo=F, results="hide", warning=F, cache=T, cache.extra=list(mtime("./Results/hays_results/skewness_results/ts_mat_CP_hays.RDS"),mtime("PPSurrogObjFun.R"),mtime("pwlin.R"),mtime("getmap.R"),mtime("alignranks.R"),mtime("SurrogsForHays.R"))}

source("SurrogsForHays.R")
#after this script runs the surrogates will be in the variable surrogs_CP_hays
```

<!--get appropriate surrogates for knz t with common+pseudo sp.-->
```{r make_surrogs_CP_knz_t, echo=F, results="hide", warning=F, cache=T, cache.extra=list(mtime("<data location in repo>"),mtime("PPSurrogObjFun.R"),mtime("pwlin.R"),mtime("getmap.R"),mtime("alignranks.R"),mtime("SurrogsForKonza_t.R"))}

source("SurrogsForKonza_t.R")
#after this script runs the surrogates will be in the variable surrogs_CP_KNZ_t
```

<!--could not get Pearson preserving surrogates for Konza f-->
...
For our purpose, we calculated average basal cover over the 36 quadrats within the exclosure surveyed for each year for each 146 species (excluding the `remove` category from 151 species list). 
