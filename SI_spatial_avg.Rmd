...
pdf("./Results/hays_results/LTmUT_hays_spaceavg.pdf",height=14,width=15)
z<-RES_hays_spaceavg$LTmUTdep
spnm_short<-c(as.character(spinfo$PLANTS.Symbol[which(spinfo$species %in% names(hays_spaceavg$avg.basal.cover))]),"pseudo_hays")
...
if(!dir.exists("./Results/hays_results/hays_npa_results")){
  dir.create("./Results/hays_results/hays_npa_results")
}
...
saveRDS(ts_knz_CP,"./Results/knz_results/skewness_results/ts_knz_CP.RDS") 
...
total_ts_C<-apply(X=ts_all_sp[,id_common_sp_knz],MARGIN = 1,FUN = sum)
total_ts_CI<-apply(X=ts_all_sp[,sort(c(id_common_sp_knz,id_normal_sp_knz))],MARGIN = 1,FUN = sum)
total_ts_CIR<-apply(X=ts_all_sp[,sort(c(id_common_sp_knz,id_normal_sp_knz,id_rare_sp_knz))],MARGIN = 1,FUN = sum)
...
if(!dir.exists("./Results/knz_results/knz_npa_results")){
...
(over all quadrats) cover data of any two 
species through out years. 
...
Fig. \ref{MT-fig_LTmUT_spaceavg_hays} in main text)
...
species pairs considered (need to add a summary table???). 
