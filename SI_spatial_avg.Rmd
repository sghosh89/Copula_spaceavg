...
for(i in 1:length(s_s)){
  ...
  # data filling for short time invasion by LASP
  if(s_s[i]=="LASP"){
    fillup<-c(0*numeric(length(1983:2013)),temp$Dat)
    temp<-data.frame(Year=c(1983:2018),Dat=fillup)
  }
...
#============================= NOTES : some formatting steps ==========================================================
# updating sp name code
i_BRSP<-which(names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear)=="BR__")
names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear)[[i_BRSP]]<-"BRSP"

# merging TRs : ("TRAL","TRSP","TRTR")
all_TR<-(data_jrg_spaceavg$jrg_spaceavg_cover_byyear$TRAL$Dat+data_jrg_spaceavg$jrg_spaceavg_cover_byyear$TRSP$Dat+data_jrg_spaceavg$jrg_spaceavg_cover_byyear$TRTR$Dat)

#=======overwrite TRSP data with total data from all Trifolium sp.=============
data_jrg_spaceavg$jrg_spaceavg_cover_byyear$TRSP$Dat<-all_TR 

#=================== and delete the TRAL and TRTR ones==============================
id_TRTRs<-which(names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear)%in%c("TRTR","TRAL"))
data_jrg_spaceavg$jrg_spaceavg_cover_byyear<-data_jrg_spaceavg$jrg_spaceavg_cover_byyear[-id_TRTRs]

#--------------------------time series plot for all sp with data_jrg_spaceavg-------------------------
summary_raw_data_spaceavg<-data.frame(sp=names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear),n=NA,n0=NA,nT=NA,nrep0=NA,cover=NA)
...
bad_id_jrg<-which(names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear) %in% c("BARE","ROCK","THSP","MISP")) # I drop for now this MISP

id_common_sp_jrg<-which(summary_raw_data_spaceavg$n0<=6)
id_common_sp_jrg<-setdiff(id_common_sp_jrg,bad_id_jrg) #if any

id_not_common_sp_jrg<-setdiff(c(1:length(data_jrg_spaceavg$jrg_spaceavg_cover_byyear)),id_common_sp_jrg)
id_not_common_sp_jrg<-setdiff(id_not_common_sp_jrg,bad_id_jrg)

id_rare_sp_jrg<-which(summary_raw_data_spaceavg$n0>=34)
id_rare_sp_jrg<-setdiff(id_rare_sp_jrg,bad_id_jrg)
...
sp_jrg_category<-as.data.frame(matrix(NA,nrow=tot_sp_jrg,ncol=3))
colnames(sp_jrg_category)<-c("id","sp","category")
sp_jrg_category$id<-c(1:tot_sp_jrg)
...
all_goodsp_jrg_category<-na.omit(sp_jrg_category)
all_goodsp_jrg_category<-all_goodsp_jrg_category[,c(2:3)]
saveRDS(all_goodsp_jrg_category,"./Results/jrg_results/skewness_results/all_goodsp_jrg_category.RDS")
#----------------------------------------------------------------------------------------------
...
i<-which(names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear)=="BRMO")
j<-which(names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear)=="PLER")
...
i<-which(names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear)=="HELU")
j<-which(names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear)=="LOSU")
d_allsp<-data_jrg_spaceavg
s1<-vivj_matrix(d_allsp=d_allsp,loc=1,i=i,j=j,ploton=F,timeavg=F,tagon=T)
...
op<-par(mfrow=c(1,2),mar=c(4,5,3,3))
i<-which(names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear)=="HELU")
j<-which(names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear)=="MIDO")
d_allsp<-data_jrg_spaceavg
...
op<-par(mfrow=c(1,2),mar=c(4,5,3,3))
i<-which(names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear)=="LOSU")
j<-which(names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear)=="MIDO")
d_allsp<-data_jrg_spaceavg
...
```{r skewness_jrg_spaceavg, echo=F, results="hide", warning=F, cache=T, cache.extra=list(seed,data_jrg_spaceavg,mtime("skewness_cvsq_testing.R"),mtime("ncsurrog.R"),mtime("mycvsq.R"),mtime("SkewnessAnd3CentMom.R"))}
...
set.seed(seed)
source("skewness_cvsq_testing.R")

# Generate matrix which holds timeseries for all 41 species along each of 41 column : 36 by 41 matrix
splist<-c(1:length(data_jrg_spaceavg$jrg_spaceavg_cover_byyear))
ts_mat<-c()
for(i in splist){
...
saveRDS(ts_mat_all_sp_jrg,"./Results/jrg_results/skewness_results/ts_mat_all_goodsp_jrg.RDS")
...
id_others<-setdiff(splist,id_common_sp_jrg)
...
id_others<-setdiff(splist,id_common_and_normal_sp_jrg)
id_pseudo<-setdiff(id_others,bad_id_jrg)
ts_mat_pseudo<-ts_mat[,id_pseudo]
ts_pseudo<-apply(ts_mat_pseudo,MARGIN = 1,FUN = sum)
...
ans<-skewness_cvsq_testing(ts_matrix = mymat,splist = c(1:ncol(mymat)),numsurrog = 10000, ploton=F, 
...
mtext(paste0("p = ",round(ans$p_left_skw,4),sep=""), side = 3, line=-2, adj=0.7,cex=2)
par(op)
dev.off()

pdf(paste(resloc2,"cvsq_jrg_spaceavg_spearman_sp_",ncol(mymat),".pdf",sep=""),height=6,width=6)
op<-par(mar=c(5.1, 5.1, 1.1, 2.1))
hist(ans$surrogcvsq,breaks = 100,main="",xlab="Surrogate cvsq",ylab="Frequency",cex.lab=2,cex.axis=2,col="grey",border=F)
abline(v=ans$realcvsq,col="black")
mtext(paste0("p = ",round(ans$p_left_cvsq,4),sep=""), side = 3, line=-2, adj=0.7,cex=2)
par(op)
dev.off()
...
pdf(paste(resloc2,"cvsq_jrg_spaceavg_spearman_sp_",ncol(mymat),".pdf",sep=""),height=6,width=6)
op<-par(mar=c(5.1, 5.1, 1.1, 2.1))
hist(ans$surrogcvsq,breaks = 100,main="",xlab="Surrogate skewness",ylab="Frequency",cex.lab=2,cex.axis=2,col="grey",border=F)
abline(v=ans$realcvsq,col="black")
mtext(paste0("p = ",round(ans$p_left_cvsq,4),sep=""), side = 3, line=-2, adj=0.7,cex=2)
par(op)
dev.off()
...
```{r skewness_hays_spaceavg, echo=F, results="hide", warning=F, cache=T, cache.extra=list(seed,hays_array,avg_cover_over_plots_hays,mtime("skewness_cvsq_testing.R"),mtime("ncsurrog.R"),mtime("mycvsq.R"),mtime("SkewnessAnd3CentMom.R"))}
...
pdf(paste(resloc2,"cvsq_hays_spaceavg_spearman_sp_",ncol(mymat),".pdf",sep=""),height=6,width=6)
op<-par(mar=c(5.1, 5.1, 1.1, 2.1))
hist(ans$surrogcvsq,breaks = 100,main="",xlab="Surrogate cvsq",ylab="Frequency",cex.lab=2,cex.axis=2,col="grey",border=F)
abline(v=ans$realcvsq,col="black")
mtext(paste0("p = ",round(ans$p_left_cvsq,4),sep=""), side = 3, line=-2, adj=0.7,cex=2)
...
library(kableExtra)
library(dplyr)
tab_spinfo<-as.data.frame(matrix(NA,nrow=length(data_jrg_spaceavg$jrg_spaceavg_cover_byyear),ncol=3))
colnames(tab_spinfo)<-c("ID","Sp. code","Species name")
tab_spinfo$ID<-c(1:length(data_jrg_spaceavg$jrg_spaceavg_cover_byyear))
tab_spinfo$`Sp. code`<-names(data_jrg_spaceavg$jrg_spaceavg_cover_byyear)
sp_jrg_full_des<-read.csv("./Data/jrg_data/datadescription_Lauren_Apr24_2019/JR_speciesnames.csv")
splistfullnm<-c("Agoseris heterophylla","Minuartia douglasii","Astragalus gambelianus",
                "Bare ground","Brodiaea spp.","Bromus hordeaceus","Bromus terrestris","Bromus berteroanus",
                "Calandrinia ciliata","Calycadenia multiglandulosa","Chlorogalum pomeridianum","Clarkia rubicunda",
                "Epilobium brachycarpum","Eschscholzia alifornica","Hesperevax sparsiflora","Gilia clivorum",
                "Hemizonia congesta","Lasthenia californica","Layia platyglossa", "Lactuca serriola",
                "Lepidium nitidum","Leptosiphon parviflorus","Lolium multiflorum","Lotus wrangelianus",
                "Lomatium utriculatum","Melica californica","Micropus californicus","Microseris douglasii",
                "Miscellaneous ?","Castilleja densiflora","Plantago erecta","Plagiobothrys nothofulvus","Poa secunda",
                "Rock","Elymus multisetus","Stipa pulchra","Thatch",
                "Crassula connata","Trifolium sp.","Vulpia microstachys"
)

#================= old one ==============================
#"AGHE" "ARDO" "ASGA" "BARE" "BR__" "BRMO" "BRTE" "BRTR" "CACI" "CAMU" "CHPO" "CLRU" "EPPA" "ESCA" "EVSP" "GICL" "HELU" "LACA" "LAPL" "LASP" "LENI" "LIAN"
# "LOMU" "LOSU" "LOUT" "MECA" "MICA" "MIDO" "MISP" "ORDE" "PLER" "PLNO" "POSC" "ROCK" "SIJU" "STPU" "THSP" "TIER" "TRAL" "TRSP" "TRTR" "VUMI"
#splistfullnm<-c("Agoseris heterophylla","Minuartia douglasii","Astragalus gambelianus",
...
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_and_normal_sp_34/skewness_jrg_spaceavg_spearman_sp_34.pdf}
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_and_normal_sp_34/skewness_jrg_spaceavg_spearman_plotcheckon_sp_34.pdf}\\
\textbf{ \hspace{0.5 cm} (G) \hspace{3.5 cm} (H)} \\
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_and_normal_and_pseudo_sp_35/skewness_jrg_spaceavg_spearman_sp_35.pdf}
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_and_normal_and_pseudo_sp_35/skewness_jrg_spaceavg_spearman_plotcheckon_sp_35.pdf}\\
\textbf{ \hspace{0.5 cm} (I) \hspace{3.5 cm} (J)} \\
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/all_sp_36/skewness_jrg_spaceavg_spearman_sp_36.pdf}
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/all_sp_36/skewness_jrg_spaceavg_spearman_plotcheckon_sp_36.pdf}\\
\caption[Skewness results for grassland community in Jasper ridge]{Skewness results for Jasper ridge; each row represents two histograms : one for comparing the position of skewness of data with its spearman preserving $10000$ surrogates having "Normal" copula structure, the other is for checking if the approximated positive semi-definite covariance matrix is acceptable or not (for details see \ref{skew_ncsurrog}). Figures are for $(A-B)$ 23 common species, $(C-D)$ 23 common sp. and rest aggregated into one pseudo sp., $(E-F)$ common and intermediate (total 34) sp., $(G-H)$ 35 (common and intermediate) sp. and rest aggragated into one pseudo sp., $(I-J)$ all 36 species including rare ones.\label{fig_jrg_spaceavg_skewness}}
\end{center}
\end{figure}

<!--jrg : skewness and cvsq results plot -->
\begin{figure}[!h]
\begin{center}
\textbf{ \hspace{0.5 cm} (A) \hspace{3.5 cm} (B) \hspace{3.5 cm} (C)} \\
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_sp_23/skewness_jrg_spaceavg_spearman_sp_23.pdf}
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_sp_23/skewness_jrg_spaceavg_spearman_plotcheckon_sp_23.pdf}
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_sp_23/cvsq_jrg_spaceavg_spearman_sp_23.pdf}\\
\textbf{ \hspace{0.5 cm} (D) \hspace{3.5 cm} (E) \hspace{3.5 cm} (F)} \\
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_and_pseudo_sp_24/skewness_jrg_spaceavg_spearman_sp_24.pdf}
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_and_pseudo_sp_24/skewness_jrg_spaceavg_spearman_plotcheckon_sp_24.pdf}
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_and_pseudo_sp_24/cvsq_jrg_spaceavg_spearman_sp_24.pdf}\\
\textbf{ \hspace{0.5 cm} (G) \hspace{3.5 cm} (H) \hspace{3.5 cm} (I)} \\
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_and_normal_sp_34/skewness_jrg_spaceavg_spearman_sp_34.pdf}
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_and_normal_sp_34/skewness_jrg_spaceavg_spearman_plotcheckon_sp_34.pdf}
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_and_normal_sp_34/cvsq_jrg_spaceavg_spearman_sp_34.pdf}\\
\textbf{ \hspace{0.5 cm} (J) \hspace{3.5 cm} (K) \hspace{3.5 cm} (L)} \\
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_and_normal_and_pseudo_sp_35/skewness_jrg_spaceavg_spearman_sp_35.pdf}
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_and_normal_and_pseudo_sp_35/skewness_jrg_spaceavg_spearman_plotcheckon_sp_35.pdf}
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/common_and_normal_and_pseudo_sp_35/cvsq_jrg_spaceavg_spearman_sp_35.pdf}\\
\textbf{ \hspace{0.5 cm} (M) \hspace{3.5 cm} (N) \hspace{3.5 cm} (O)} \\
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/all_sp_36/skewness_jrg_spaceavg_spearman_sp_36.pdf}
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/all_sp_36/skewness_jrg_spaceavg_spearman_plotcheckon_sp_36.pdf}
\includegraphics[width=4 cm]{./Results/jrg_results/skewness_results/all_sp_36/cvsq_jrg_spaceavg_spearman_sp_36.pdf}\\
\caption[Skewness and $CV^2$ results for grassland community in Jasper ridge]{Skewness results for Jasper ridge; each row represents 3 histograms : first one is for comparing the position of skewness of data with its spearman preserving $10000$ surrogates having "Normal" copula structure, second one is for checking if the approximated positive semi-definite covariance matrix is acceptable or not (for details see \ref{skew_ncsurrog}) and the third is for comparing the value of $CV^2$ of data with its spearman preserving $10000$ surrogates having "Normal" copula structure. Figures are for $(A-C)$ 23 common species, $(D-F)$ 23 common sp. and rest aggregated into one pseudo sp., $(G-I)$ common and intermediate (total 35) sp., $(J-L)$ 35 (common and intermediate) sp. and rest aggragated into one pseudo sp., $(M-O)$ all 39 species including rare ones.\label{fig_jrg_spaceavg_skewness_cvsq}}
\end{center}
\end{figure}
...
<!--hays : skewness + cvsq results plot -->
\begin{figure}[!h]
\begin{center}
\textbf{ \hspace{0.5 cm} (A) \hspace{3.5 cm} (B) \hspace{3.5 cm} (C)} \\
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/common_sp_20/skewness_hays_spaceavg_spearman_sp_20.pdf}
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/common_sp_20/skewness_hays_spaceavg_spearman_plotcheckon_sp_20.pdf}
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/common_sp_20/cvsq_hays_spaceavg_spearman_sp_20.pdf}\\
\textbf{ \hspace{0.5 cm} (D) \hspace{3.5 cm} (E) \hspace{3.5 cm} (F)} \\
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/common_and_pseudo_sp_21/skewness_hays_spaceavg_spearman_sp_21.pdf}
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/common_and_pseudo_sp_21/skewness_hays_spaceavg_spearman_plotcheckon_sp_21.pdf}
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/common_and_pseudo_sp_21/cvsq_hays_spaceavg_spearman_sp_21.pdf}\\
\textbf{ \hspace{0.5 cm} (G) \hspace{3.5 cm} (H) \hspace{3.5 cm} (I)} \\
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/common_and_normal_sp_100/skewness_hays_spaceavg_spearman_sp_100.pdf}
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/common_and_normal_sp_100/skewness_hays_spaceavg_spearman_plotcheckon_sp_100.pdf}
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/common_and_normal_sp_100/cvsq_hays_spaceavg_spearman_sp_100.pdf}\\
\textbf{ \hspace{0.5 cm} (J) \hspace{3.5 cm} (K) \hspace{3.5 cm} (L)} \\
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/common_and_normal_and_pseudo_sp_101/skewness_hays_spaceavg_spearman_sp_101.pdf}
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/common_and_normal_and_pseudo_sp_101/skewness_hays_spaceavg_spearman_plotcheckon_sp_101.pdf}
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/common_and_normal_and_pseudo_sp_101/cvsq_hays_spaceavg_spearman_sp_101.pdf}\\
\textbf{ \hspace{0.5 cm} (M) \hspace{3.5 cm} (N) \hspace{3.5 cm} (O)} \\
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/all_sp_146/skewness_hays_spaceavg_spearman_sp_146.pdf}
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/all_sp_146/skewness_hays_spaceavg_spearman_plotcheckon_sp_146.pdf}
\includegraphics[width=4 cm]{./Results/hays_results/skewness_results/all_sp_146/cvsq_hays_spaceavg_spearman_sp_146.pdf}\\
\caption[Similar figure as of Fig. \ref{fig_jrg_spaceavg_skewness_cvsq} but for Hays dataset.]{Similar figure as of Fig. \ref{fig_jrg_spaceavg_skewness_cvsq} but for Hays dataset. Figures are for $(A-C)$ 20 common species, $(D-F)$ 20 common sp. and rest aggregated into one pseudo sp., $(G-I)$ common and intermediate (total 100) sp., $(J-L)$ 100 (common and intermediate) sp. and rest aggragated into one pseudo sp., $(M-O)$ all 146 species including rare ones though histogram $(J)$ did not validate the approximated version of positive semi-definite covariance matrix.\label{fig_hays_spaceavg_skewness_cvsq}}
\end{center}
\end{figure}
