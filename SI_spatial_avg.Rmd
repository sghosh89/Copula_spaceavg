...
```{r make_surrogs_CP_hays, echo=F, results="hide", warning=F, cache=T, cache.extra=list(ts_mat_CP_hays,mtime("PPSurrogObjFun.R"),mtime("pwlin.R"),mtime("getmap.R"),mtime("alignranks.R"))}
...
try(res<-constrOptim(theta=pijstart,f=PPSurrogObj,grad=NULL,ui=ui,ci=ci,
                 sumvij=sumvij,mapx=mapx,mapy=mapy,saveloc=resloc_surrog_hays,
                 control=list(fnscale=-1,trace=0,maxit=10000)),
    silent=TRUE)

#***now if it succeeded in finding a pos def mat, then it saved a result, and 
#we can pull the result backin from the disk and examine it
...
}else
{
  ...
  plot(vij,pij,type='p',xlab="vij",ylab="pij",xlim=c(-1,1),ylim=c(-1,1))
  text(1,1,cor(vij,pij),adj=c(1,1))
  dev.off()
}
...
pdf(file=paste0(resloc_surrog_hays,"CVcom2_For_Surr_AndDat.pdf"))
hist(CVcom2_sur,main=sum(survars<var(totd))/10000)
