...
pseudo_hays_IR<-apply(X=avg_cover_over_plots_hays[,which(sp_hays_category$category%in%c("I","R"))],MARGIN = 1,FUN = sum)
...
ts_all_sp<-matrix(NA,nrow=lenyr,ncol=lenusp) # timeseries for each sp along each column
colnames(ts_all_sp)<-usp
rownames(ts_all_sp)<-uyr
ts_all_sp<-as.data.frame(ts_all_sp)

my_knz<-dat_knz[,c("year","species","abundance")]
my_knz_eachsp<-split(my_knz,my_knz$species)

all(names(my_knz_eachsp)==colnames(ts_all_sp))==T # It's a check

for(i in 1:length(my_knz_eachsp)){
  x1<-my_knz_eachsp[[i]]
  ts_all_sp[,i]<-sapply(split(x=x1$abundance,f=x1$year), mean)
}
saveRDS(ts_all_sp,"./Results/knz_results/ts_all_sp.RDS")

anyNA(ts_all_sp)

# screen sp. for KNZ
nyr_0_eachsp<-apply(ts_all_sp,MARGIN = 2,FUN = function(x){sum(x==0)})
...
ts_common_knz<-ts_all_sp[,id_common_sp_knz]

# rare sp. for KNZ
id_rare_sp_knz<-which(nyr_0_eachsp$nyr_0_eachsp>=31)
ts_rare_knz<-ts_all_sp[,id_rare_sp_knz]

# normal or intermediate sp. for KNZ
id_normal_sp_knz<-which(nyr_0_eachsp$nyr_0_eachsp>0 & nyr_0_eachsp$nyr_0_eachsp<31)
ts_normal_knz<-ts_all_sp[,id_normal_sp_knz]

# sp. category for KNZ
sp_category_knz<-data.frame(sp=rownames(nyr_0_eachsp),category=NA)
sp_category_knz$category[id_common_sp_knz]<-"C"
sp_category_knz$category[id_normal_sp_knz]<-"I"
...
# Append the pseudo species = merged sp. of I & R category
pseudo_knz_IR<-apply(X=ts_all_sp[,which(sp_category_knz$category%in%c("I","R"))],MARGIN = 1,FUN = sum)
...
ts_knz_CP<-cbind(ts_common_knz,pseudo_knz_IR)
saveRDS(ts_knz_CP,"./Results/knz_results/ts_knz_CP.RDS") 
...
```{r ms_knz_spaceavg,echo=F, results="hide", cache=T, cache.extra=list(seed,knz_spaceavg,mtime("FittingCopula_ms.R"), mtime("vivj_matrix.R"),mtime("OurBiCopSelect.R"),mtime("MyBiCopGofTest.R"),mtime("preprocessors.R"))}
source("./FittingCopula_ms.R") # top most hierarchy source file
...
RES_knz_spaceavg<-RES_single_loc(loc=1,d_allsp=knz_spaceavg,
                                  good_sp=c(1:length(knz_spaceavg[[1]])),
                                  families=c(1,3:10,13,14,16:20),level=0.05,timeavg=F)

saveRDS(RES_knz_spaceavg,"./Results/knz_results/RES_knz_spaceavg.RDS")
```

```{r plot_LTmUT_knz_spaceavg, echo=F,results="hide",message=F}
set.seed(seed)
source("./mycorrplot.R")

pdf("./Results/knz_results/LTmUT_knz_spaceavg.pdf",height=15,width=16)
z<-RES_knz_spaceavg$LTmUTdep
spnm_short<-names(knz_spaceavg$avg.percent.cover)
rownames(z)<-paste0(rownames(z)," : ",spnm_short,sep="")

colnames(z)<-rownames(z)
cmin<-min(RES_knz_spaceavg$LTmUTdep,na.rm=T)
cmax<-max(RES_knz_spaceavg$LTmUTdep,na.rm=T)
cr<-max(abs(cmin),abs(cmax))
mycorrplot(z=z,
           posnI_ind = RES_knz_spaceavg$posnI_ind,
           posnN_ind = RES_knz_spaceavg$posnN_ind,
           colrange = c(-cr,cr),
           nsm=RES_knz_spaceavg$neg_sp_mat)
nvar<-0
dl2<-nrow(z)-nvar
z1<-z
z1[RES_knz_spaceavg$posnN_ind]<-NA
z1<-z1[1:dl2,1:dl2]
nL<-sum(z1>0,na.rm = T)
nU<-sum(z1<0,na.rm = T)
mtext(paste0("nL =",nL,", nU =",nU),cex=3,side=1,line=2.8)
...
if(!dir.exists("./Results/knz_resultsknz_npa_results")){
  dir.create("./Results/knz_results/knz_npa_results")
}

resloc<-"./Results/knz_results/knz_npa_results/"
loc<-1

stat_knz_npa_spaceavg<-vector("list",1)
names(stat_knz_npa_spaceavg)<-"knz_spaceavg"

if (!dir.exists(paste(resloc,names(stat_knz_npa_spaceavg),sep=""))){
  dir.create(paste(resloc,names(stat_knz_npa_spaceavg),sep=""))
}

stat_knz_spaceavg<-multcall(d_allsp=knz_spaceavg,
                             loc=1,
                             pfname=paste(resloc,names(stat_knz_npa_spaceavg),"/",names(stat_knz_npa_spaceavg),sep=''),
                             good_sp=c(1:length(knz_spaceavg[[1]])),
                             nbin=2,timeavg=F)

stat_knz_npa_spaceavg[[loc]]<-stat_knz_spaceavg

saveRDS(stat_knz_npa_spaceavg,paste(resloc,names(stat_knz_npa_spaceavg),"/",file="stat_knz_npa_spaceavg.RDS",sep=''))
```

```{r plot_res_knz_npa_spaceavg, echo=F, results="hide",cache=T,warning=F, cache.extra=list(seed,stat_knz_npa_spaceavg,mtime("NonParamStat_matrixplot.R"),mtime("mycorrplot.R"))}
set.seed(seed)
source("./NonParamStat_matrixplot.R")
r<-readRDS("./Results/knz_results/RES_knz_spaceavg.RDS")
resloc<-"./Results/knz_results/knz_npa_results/"

nvar<-0
nvar_names<-NA

npa_diff_knz_spaceavg<-NonParamStat_matrixplot(data_ln_all = stat_knz_npa_spaceavg,
                                                posnI_list_ln_all = list(r$posnI_ind),
                                                posnN_list_ln_all = list(r$posnN_ind),
                                                resloc=paste(resloc,sep=""),
                                                nvar=nvar,nvar_names=nvar_names,r=list(r),tagon=F)
saveRDS(npa_diff_knz_spaceavg,paste(resloc,file="npa_diff_knz_spaceavg.RDS",sep=''))
```
...
library(kableExtra)
library(dplyr)
tab_spinfo<-as.data.frame(matrix(NA,nrow=21,ncol=3))
colnames(tab_spinfo)<-c("ID","Sp. code","Species name")
tab_spinfo$ID<-c(1:21)
tab_spinfo$`Sp. code`<-c(spinfo$PLANTS.Symbol[which(spinfo$species %in% names(hays_spaceavg$avg.basal.cover))],"pseudo")
...
\includegraphics[width=6 cm]{./Results/hays_results/skewness_results/total_timeseries.pdf}\\
\caption[Timeseries plot for (A) Jasper Ridge and $(B)$ Hays data]{ Timeseries plot for (A) Hays and $(B)$ Hays data; "common" species indicate the most abundant species
