...
<!--New investigations of empirical cdfs-->
```{r empcdfs, echo=T, results="show"}
#***DAN: Shyamolina, I want to do a few additional computations here. Can you please indicate the names of, in a comment, 
#or create and then
#indicate the names of, variables which are available in the workspace at this point in the code that have the following:
#1) Year by species, population abundance matrix for Hays, including all 18 real species and the psuedo-species - so this is 
#just the main data matrix. It's OK if it's species/pseudospecies by year instead (i.e., I don't care if it's transposed)

#***Shya:
# ts_mat_CP_hays = a dataframe of 41 years by 19 sp.=(18+1pseudo)
# please convert to matrix with as.matrix(ts_mat_CP_hays)

#2) The same for Konza (but 22 species/pseudospecies)

#***Shya:
# ts_mat_CP_knz = a dataframe of 36 years by 22 sp.=(21+1pseudo)
# please convert to matrix with as.matrix(ts_mat_CP_knz)

#I am sure these already exist in some form, and I suspect at this point in the code they are available, I just want to know what the
#variables are, for sure, from you, to make sure I do not start my work with the wrong variables. If these are stored in some
#form other than a matrix, please write code below to create matrices as specified. Please make the row names be the sampling years
#and the column names be the species names.
#BTW, if you need to move this chunk to a better location within the code to facilitate doing what I'm asking for, go ahead and 
#move it
#I also am going to need variables in this form:
#3) Year by species by surrogate array with surrogate species abundance data for Hays
#4) The same for Konza
#So 3 and 4 are just the surrogates for 1 and 2, respectively. I need all 10000 surrogates here, please.

#***Shya: following two vaiables are array (41x19x10000 for Hays, 36x22x10000 for knz), 10000 sampled from the initially saved 10^5 surrogs, seed was used 101 to sample. The following two variables we used to calculate stability indices in previous chunks. Each array contains a matrix (year by species) surrogate of actual data.

# surrogs_CP_hays_sampled
# surrogs_CP_knz_sampled
```
...
<!--# Supporting figures-->
<!--***DAN: Shyamolina, currently when I knit this I get the header "Supporting figures" and then, on a new page,
the references, and then on a new page, the figures, and then the tables. Please fix. Please also make "Supporting 
tables" be a section, containing the tables.

***Shya: I can't, I remember the similar problem we faced in our BIVAN paper. 
You have two choice as far as I can remember (I am attaching two version in my email):
1) You can get a section heading, but figures/ table will be put after ref as you are using endfloat package in rmd,
so, this is what now you are getting.
2) without any section heading at all (see attached option2suppmat.pdf), like our BIVAN paper, I think suppmat does not need any specified formatting and I prefer this.
3)I can put a tag (not a section number, and that too will not look good on the first appearence), see atached option3suppmat.pdf
--->
...
```{r reformatting_figs, echo=F, results="hide"}
...
\end{figure}
<!--***DAN: Shyamolina, can you please modify the code that makes this figure so that the following changes 
are implemented:
1) Make the x-axis labels say "Normal-copula parameter, p"
2) Make the y-axis labels say a latex rendering of \varphi(p)
3) Make the axis labels and numbers, for both the x and y axes, be at least twice as big as they currently are.
Also, the code should be configured so it runs when a knit is performed - I can't remember if that's been done
or if this code is still kept separate.
...
\caption{Results show graphical representation of Fig. \ref{MT-fig_method_box} $(A-D)$ with the actual communities, 
