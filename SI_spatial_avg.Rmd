...
upper-tail dependent noise, normally distributed
for each location. If lower-tail dependent noise is desired, take the negative of the
entire matrix.-->
...
\caption[Timeseries plot for (A) Hays and (B) Konza data]{Timeseries plot for (A) Hays and (B) Konza data; "common" species indicate the most abundant species throughout years averaging over quadrats and present minimum for $35$ years. "Rare" species refer to the species which was present maximum for 2 years in both datasets. The rest are classified into "intermediate" category. The total percent cover in (B) are often beyond 100 because we considered the highest cover for each species when they were sampled multiple times in a year, therefore, if some species were dominating (say, >50) during first sampling time and others during the second sampling time for a given year, the aggregated effect yields more than 100$\%$ cover for that year.\label{fig_spaceavg_timeseries}}
\end{center}
\end{figure}
...
-->
<!--***DAN: Shyamolina, please specify the units for panel A
***Shya: Done.-->
