...
knitr::kable(tab_spinfo_hays, 
             ...
             caption = "`Common' species (see Methods) of the Hays dataset. The column `ID' is the species ID used in Fig. \\ref{MT-fig_CorlmCoru}. The column `Sp. code' is the species code used in the downloaded, raw data. \\label{tab_spinfo_hays}", 
...
knitr::kable(tab_spinfo_knz, 
             ...
             caption = "`Common' species (see Methods) of the Konza dataset. The column `ID' is the species ID used in Fig. \\ref{MT-fig_CorlmCoru}. The column `Sp. code' is the species code used in the downloaded, raw data.\\label{tab_spinfo_knz}", 
             booktabs = T,col.names = NULL) %>%
             column_spec(3,italic=T)%>%
             add_header_above(c("ID"=1,"Sp. code"=1,"Species name"=1))
```
...
***Shya: I don't think it is a mistake. The percent cover should not take literally as the cover is computed based on midpoint rule of each of the seven "class" categories: see details already explained in the Data section (line 909 of this Rmd). If you agree with me I will write something like this in the figure caption.-->
