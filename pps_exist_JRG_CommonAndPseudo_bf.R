...
d<-readRDS("./Results/jrg_results/skewness_results/ts_mat_all_goodsp_jrg.RDS")# for all species except in "REMOVE" category

#Importing data with "C", "I", "R" values for common, intermediate, and rare species
category_jrg<-readRDS("./Results/jrg_results/skewness_results/all_goodsp_jrg_category.RDS")
all(colnames(d)==category_jrg$sp)==T
src<- category_jrg$category #a vector with one entry for each species, corresponding to the columns of dHAY
...
#some of these come very close to the edges of the hlo-hhi range
