...
Thus $\CVcomsq=\phicv \CVindsq$. Because $\CVindsq$ is interpreted as what community variability would be, without species interactions,
$\phicv$ is the factor by which species interactions inflate or decrease community variability.
...
have also been proposed [@loreau2008species]. However, $\phicv$ is still commonly used; 
...
were the same in both scenarios, and so $\phicv$ was also the same. The Loreau-de Mazancourt variance ratio
...
Just as, in the classic approach, the variance ratio $\phicv$ is the quotient $\CVcomsq/\CVindsq$, we 
...
the absence of species interactions, i.e., $\Sind$. In such a case, and assuming for simplicity $\phicv \approx 1$
...
$\phicv \approx 1$ -- see Discussion). This can be viewed as a new type of compensatory dynamics, in the sense that species 
...
The skewness ratio $\phiS$ provides information about the influence of species interactions, of all kinds,
on the skewness of $\xtot$; this is because $\phiS$ is the quotient of $\Scom$, which is influenced by species interactions
...
It is straightforward to show from the definitions that $\phiS=\phiSta\phiScor$, so
$\Scom=\phiSta \phiScor\Sind$.
Thus the two newly defined quantities, $\phiSta$ and $\phiScor$, separate the influence of 
...
of the skewness ratio due to tail association, $\phiSta$, was less than 1,
i.e., $\phiSta=0.2757<1$. 
...
so for Konza $\phiSta$ was greater than 1 (it was $2.7999$). 
<!--***DAN: Shyamolina, please make the above number an autolink-->
...
Because $\phicv<1$ (Fig. \ref{fig_result_box}), the conclusion solely based on the classic 
...
Because $\phicv$ was slightly less than $1$ for Konza (Fig. \ref{fig_result_box}), the conclusion solely based on the classic 
...
ratio alone does not  reveal. For both communities, $\phicv$ is slightly or moderately less than $1$,
...
$\phiScor$ and $\phiSta$ were similar (Fig. \ref{fig_result_box}), indicating that both tail associations and
...
of $\xtot$ (recall that $\Scom=\phiSta \phiScor \Sind$). However, for Hays, $\phiScor$
...
\end{center}
\end{figure}
<!--***DAN: Shyamolina, in this fig, in panels A and B, are written two instances of 
\phi_cv. Please change them to \phi_CV-->
