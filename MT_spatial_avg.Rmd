...
options(scipen = 1, digits = 4) #This option round all numbers appeared in the inline r code upto specified digit
```
...
The standard deviation of $\xtot$ was about $15.7$ 
<!--`r round(sqrt(vartotX1),1)`
***Shya: I could not find vartotX1 variable-->, and the skewness of $\xtot$
...
Results revealed marked differences between the two communities in patterns of species tail associations. 
...
without tail associations (Methods). We tested these hypotheses for Hays and Konza, thereby exploring the influence
...
significantly modify the skewness of the total community abundance, $\xtot$, either lowering it (lower-tail asociations, as in Hays)
or raising it (upper-tail associations, as in Konza) and thereby having an influence on patterns of community variability.  
...
only the variance ratio approach, to conclusions drawn using both the variance and skewness ratio approaches.
...
(it was `r hays_stab$phi_skw`), with $\Scom=$ `r hays_stab$skw_real` less than $\Sind=$ `r hays_stab$skw_indep`(Fig. \ref{fig_result_box}).
...
<!--***DAN: Shyamolina, please make the above three numbers into autolinks
***Shya: done. but see sometimes decimal numbers are not exactly the same as in figure because the saved table considers the full numbers to calculate phi_cv whereas phi_cv in figures are calculated with the truncated values.-->
...
modify extreme bahavior of $\xtot$ very differently in the two tails of its distribution. The extent to which it makes 
sense to say that species interactions promote community stability for Konza is less than what is implied
by the variance ratio. See SI section \ref{ecdf_approach} 
for more results and further discussion.

Thus combining approaches newly reveals differences between Hays and Konza.
...
of $\xtot$ differs markedly between Hays and Konza. Hays can probably still be fairly labeled as a community where
compensatory dynamics are dominant, and species interactions stabilize community dynamics. But, in contrast,
species interactions in Konza only appear to be compensatory in the narrow viewpoint of the variance ratio
approach. Our broader approach that considers both variance and skewness, and that assesses the influence of 
species interactions on both crashes and explosions of $\xtot$, reveals that the situation is more complex
and it is probably not fair to state, without futher qualification, that species interactions are stabilizing in 
Konza.  

Finally, our results highlight the importance of tail associations for community dynamics. 
The influence of tail associations on distributions of $\xtot$ can be
as great or greater than the influence of aspects of species interactions other than tail association. For Konza, 
$\phiScor$ and $\phiSta$ were similar (Fig. \ref{fig_result_box}), indicating that both tail associations and
aspects of species interactions other than tail associations were comparable in their effects on the skewness
...
influence of species interactions on the skewness of $\xtot$ is due to the nature of tail associations between species. 
The influence of tail associations on the CV of $\xtot$ is negligible, as indicated previously, but reflects
the inability of classical approaches to detect the importance of tail associations rather than indicating that
