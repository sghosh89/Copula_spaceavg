...
<!--background-->
The degree of synchrony among the population fluctuations of multiple 
coexisting species is known to have a major impact on community stability, i.e., the stability, 
through time, of aggregate community properties such as total community abundance or biomass. 
However, synchrony and its influence on stability are usually measured 
using covariance methods. Such approaches do not account for the possibility that species abundances may be 
more correlated when the species are relatively common than when they are relatively scarce, or vice versa. 
...
community stability. 
<!--new methods-->
We propose using skewness of the aggregate community property time series as an alternative 
measure of community stability which complements and extends standard measures. We develop a “skewness 
ratio” which quantifies the extent to which relationships between species, and asymmetric tail associations in 
these relationships, serve to modify community stability. The skewness ratio is analogous 
to, but complements, the classic variance ratio and related metrics. Using two multi-decadal grassland datasets, we 
show that taking into account asymmetric tail associations gives new viewpoints on the relationship 
between synchrony and community stability. 
<!--results-->
For instance, we show that species associations can augment or degrade community stability differentially for
community crashes or explosions to high values, a fact which is not detectable by standard approaches.
In other words, species associations can mitigate explosions of total community abundance to high values (increasing
one aspect of community stability), while simultaneously exacerbating the
tendency to crash to low values (decreasing another aspect of stability); or vice versa.
<!--summary-->
Our theory and results constitute the beginnings of a new and more 
...
synchrony, tail association, grassland, skewness, temporal variability, compensatory dynamics
...
sense that the combined functioning of a community remains stable despite substantial species turnover [@RN7]. 
...
the variance through time of an aggregate community property, 
together with covariances between population time series, to characterize species 
relationships [@peterson1975; @schluter1984variance; @RN23; @frost1995species; @loreau2008species; @Gross2014].
...
@loreau2008species, here denoted $\phiLdM$. 
...
by any extensions or alternatives of which we are aware. Whereas the variance ratio 
...
To begin by giving an intuitive introduction to how the main concepts of this study can extend previous approaches, 
we consider two simulated community dynamics datasets 
...
and the covariances $v_{ij}$ will not detect the differences between our two scenarios. The correlation-based
metric of @Gross2014 is another such.
<!--***DAN: Don't forget to roll out my notational changes in the figures, and also make sure it's worked in
the sup mat. You'll have to read the whole sup mat again.-->
...
have different growth prospects if that mixture exhibits occasional crashes compared to if the mixture 
...
Second, commonly used past measures of community variability have focused on the variance or 
...
Our goal (G2) is to develop and apply an
approach to studying community variability that complements the traditional 
coefficient of variation.
Finally, the example above suggests that ways in which species relationships
...
can benefit from application of the same methods. Overall, this study establishes the beginnings of a more flexible paradigm for
...
applied scenarios in which exceeding a high threshold is disasterous, and other scenarios in which disaster
...
(Fig. \ref{fig_result_box}A, C), i.e., the variance ratio approach did not register the importance of 
...
by the randomized, surrogate datasets (Methods). $\CVntasq$ and $\CVcomsq$ were 
...
<!--***DAN: Interesting comparison to the LdM approach may be here (though put it in the discussion).
\phiLdM is always less than 1, leading to the interpretation that species relationships always reduce 
community variability, at least in comparison to the case of perfect linear relationships between species.
...
or "destabilizing" (i.e., reducing versus increasing community variabiluty) in the two tails 
...
tail associations were unimportant. See also below for discussion of how our 
...
$\phiLdM$ has over $\phicv$; nevertheless, such a metric may be worth considering in future work, and we are happy to admit the 
possibility that refinements of our metric $\phiS$ may later be developed incorporating the same main ideas of this study
...
kelpt constant. Species comonotonicity can be realized for data by sorting each species' time series. 
<!--and reveals the same main ideas we elaborated in this study using $\phiS$-->
The alternative approach would reveal essentially the same main ideas because $\Snta$ and $\CVntasq$ would probably have 
to be defined in the same way. The main ideas of this manuscript center on the importance of tail associations for
...
as a scaling factor converting population- to community-level variability. An analogous interpretation for 
$\phicv$ is not available. But such an interpretive advantage does not appear to extend to $\phiSLdM$, at least
not as that metric was formulated above. Decades passed between the original formulation of the variance ratio and the formulation
of the Loreau-de Mazancourt and other improvements on the variance ratio. Our goal with this study was to introduce the 
...
will reveal a metric that captures these main ideas, but with additional interpretive and other improvements over
the skewness ratio we have presented.
...
to crash to low values; or they can mitigate crashes to low values, while simultaneously exacerbating the tendency 
to explode to high values. Our new skewness ratio approach complements the variance ratio 
to reveal a more multidimensional nature of species relationships and their correspondingly 
multifarious effects on community variability, in real-world datasets. 
...
descriptions of synchrony, including but not limited to the descriptions we have
provided, may be useful elsewhere, beyond finance and community ecology. 
...
the synchrony or asynchrony of the signals. This is
true of both financial systems and of the population systems
we studied. But it seems likely to be a common feature of many 
systems. 
<!--neuron-->
For instance, a neuron may fire only when its input neurons fire simultaneously, so that total
inputs pass a threshold. 
<!--grid-->
The electrical grid may crash only when the demands of a multitude of users become synchronized,
producing total usage spikes. 
<!--economics-->
Macroeconomic measures may become depressed only when the 
multitude of microeconomic activities
that comprise them are simultaneously depressed. 
<!--insect calls-->
Female insects may be drawn to an area only when the vocalizations of males in
the area are coordinated to reinforce each other, so that calling amplitude passes a threshold. 
<!--After examples-->
In many fields concerned with synchrony, techniques for quantifying the
synchrony of individual signals and relating that synchrony to distributional properties of the 
aggregate quantity of interest have historically been crude. We believe there is great opportunity to 
improve these techniques to benefit numerous fields.
At least one paper in this special issue seems
to proceed principally by developing newly detailed descriptions of a kind of synchrony, and then using them
to make inferences about the mechanisms or consequences of synchrony: @DotovInReview
developed frequency-specific synchrony measures and used them to describe behavioral rhythm phenomena. 
@sheppard2016, @sheppard2019 and @zhao2020 also developed frequency-specific measures of synchrony
and made new inferences about population and community ecology. 
Some of the other papers in this special issue describe synchrony between two or more phenomena principally  with
a univariate description of how strong or consistent the synchrony is. 
For instance,
@LenseInReview indicate that many childhood neurodevelopmental disorders are associated deficits in 
rhythm, timing and synchrony. One wonders how such deficits are typically measured, clinically,
and whether additional inferences may be possible, or additional discoveries made, by employing more 
detailed decriptions of synchrony. 
For instance, one wonders if improved diagnosis of childhood neurodevelopmental disorders [@LenseInReview]
may eventually be possible by appropriately formulating and applying
more detailed statistical descriptors of rhythm and synchrony
tasks attempted by patients. Mathematically, 
the nature of the relationship between two quantities is an infinite dimensional object [@Ghosh_copula],
but many studies
of synchrony and related concepts have used univariate descriptions. 
It is plausible that improved results could be extracted
across many fields through the use of detailed statistics such as we have used here and elsewhere.
...
a disasterous threshold for $\xtot(t)$ to cross, then the first scenario (C, E) would involve much
...
On the other hand, if the upper threshold (dotted line) were a disasterous threshold for 
