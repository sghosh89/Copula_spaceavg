...
<!--***DAN: Shyamolina, is 1983-2019 the right range of years? In the SI section on the data, you say the data started in 1983
and were downloaded in 2019, but I don't know whether the end of the data you analyzed was 2019 or 2018 or what?
Sometimes the processed, published data are a year or more behind the actual sampling, so please check the end
year of the data you used, and if it differed from 2019 change the above line
***Shya: it is downloaded in Nov, 2019 but data were updated on their website upto 2018 as of Feb, 2019.
Every year on Feb they updated their data upto the previous year. Like if you now download their data it will be upto 2019 updated on Feb, 2020.-->
Additional details of the sites and initial processing efforts are in SI section \ref{SI-Data}.
<!--***DAN: Shyamolina, I created the above summaries after carefully reading your detailed descriptions in the SI section 
on the data, but please check them carefully to make sure I did not misinterpret anything you wrote there.
***Shya: checked.-->
...
Given two bounds $0\leq l_b < u_b \leq 1$, we define the lines $u_i + u_j=2l_b$ and $u_i+v_i=2u_b$,
...
above formula is only computed over $t$ such that $u_i(t) + u_j(t)>2l_b$ and $u_i(t) + u_j(t)<2u_b$.
The partial Spearman correlation is the component of the Spearman correlation which can be attributed to
the points in the band. For two positively associated variables, $\cor_{0,u_b}$ for $u_b \leq 0.5$ measures 
lower-tail association, and $\cor_{l_b,1}$ for $l_b \geq 0.5$ measures upper-tail association. 
...
correlations we introduce here.
...
suggest that the nature of tail associations of positively associated species may influence
...
across all positively associated species pairs, $i,j$. This was positive of lower-tail 
association was, in aggregate across the community, stronger than upper-tail association, and
negative if the reverse.
...
fluctuation, then the community is considered more stable. In mathematical notation, 
...
interspecific interactions at the extreme values can alter the skewness (i.e. changing its sign) of a 
community-biomass distribution over years than it would have been expected without any interactions. 
<!--for example, see explore_phis.R where a lower-tail dep. community 
...
\end{center}
\end{figure}
<!--***DAN: Don't forget to fill in the SI section number, X in the above-->
...
<!--***DAN: Shyamolina, can you please remove the diagonal dashed lines.
***Shya: Done.-->
<!--***DAN: Shyamolina, can you please make the x axis be labeled $u_i$ and the y axis be labeled $u_j$ on both panels? Also, please
change the panel labels to (A) and (B).
***Shya: Done.-->
