...
community with $n$ number of species - so that there are $n(n-1)/2$ number of unique 
pair-wise interspecific interactions possible. First, for each of these species-pairs, we
construct the scatter plots with the normalized rank of corresponding time-series 
<!-- omitted as of now: 
and test their 
...
tail-asymmetry for the interspecific response within the community, the larger would be 
the difference between $\phi_{s,cor}$ and $\phi_{s,ta}$. Following we will present the results 
from the analyses with two examplary long-term community data. The complete reproducible workflow 
for this project is available at https://github.com/sghosh89/Copula_spaceavg.
