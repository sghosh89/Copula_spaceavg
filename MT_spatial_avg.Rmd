...
total community biomass. The setup for the theory is as follows. Let $x_i(t)$ be an abundance 
...
whole distribution of $\xtot$, and its skewness. To describe relationships between species, 
...
in fact, ecological distinct, as we will see, in that the total biomass $\xtot$ achieves much
...
so-called *comonotonic* scenario for which the $x_i$ are all equal. This is a scenario of maximal covariance between species
...
The other scenario for comparison is the comonotonic scenario, scenario C, for which $x_i=x_1$ for all $i$. 
Comonotonicity between $x_1$ and $x_2$ is pictured in Fig.  \ref{fig_theory}E.
...
of tail comonotonicity and tail associations [@Ghosh_extinction;@Ghosh_copula], and the consequences of tail
comonotonicity for the distribution of $\xtot$ and the probability of $\xtot$ passing a threshold. 
...
below the threshold (Fig. \ref{fig_theory}I; see SI section X for details). Correlation below the threshold was selected 
...
and related approaches, the the probability of exceeding high thresholds in scenario 2 is much elevated compared 
to scenario 1, and, in fact, is the same as scenario C and hence is as high as it can possibly be. The community
...
Each $x_i$ is again standard-normally distributed (Fig. \ref{fig_theory}I side panels), but for this scenario
...
above the threshold (Fig. \ref{fig_theory}M; see SI section X for details).
...
Scenario 4 shows comonotonicity in both tails. Therefore probabilities of exceeding high thresholds and falling below
...
$(x_1,\ldots,x_N)$ with the $F_1,\ldots,F_N$ as their marginal distribution functions. Thus the Fr\'{e}chet space
represents, in our modelling context, all possible interspecific relationships (e.g., different degrees and kinds of 
synchrony and compensatory dynamics between species) that can pertain, given fixed
species marginal distributions. So the question of what values $\var(\xtot)$ can take across the Fr\'{e}chet space
is a related, but more precise version of the classic question of how species interrelationships 
...
@cheung2013 showed the converse, i.e. if $\var(\xtot)$ is maximal in the Fr\'{e}chet space, then 
$(x_1,\ldots,x_N)$ must be comonotonic (@cheung2013 made some mild regularity assumptions about the $F_i$). 
...
marginals are considered fixed). But @cheung2013 showed that, even if we consider only the sub-Fr\'{e}chet space
...
ecologically important ways. Species marginal distributions, and therefore species means and variances
...
(C-F). The total was much more likely to crash to low values in the first scenario (C, E)
than in the second scenario (D, F), whereas $\xtot(t)$ was much more likely to explode to high values 
in the second scenario than in the first. In both scenarios, there were two groups of species (red and black lines).
Species in different groups were were compensatory, but for scenario 1, within-group synchrony occurred principally
when populations were low, and for scenario 2 when populations were high. See the Introduction and SI section X for notation and 
additional details. Time series were generated and statistics computed for 100000 years, but are plotted on A, B
for 60 years only, for clarity. Dashed and dotted lines show lower and upper thresholds which may be considered
disasterous for $\xtot(t)$ to cross. Probabilities of crossing these thresholds were negligible for one scenario, but
non-negligible for the other.\label{fig_pedag_cv2_skw}}
\end{center}
\end{figure}
...
\caption{Fill in. \label{fig_theory}}
\end{center}
\end{figure}
<!--***DAN: Don't forget to write this, Dan.-->
