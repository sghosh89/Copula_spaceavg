...
to, but complements, the classic variance ratio. Using two multi-decadal grassland datasets, we 
...
Because $\CVindsq$ is interpreted as what community variability would be, without species relationships,
$\phicv$ is the factor by which species relationships inflate or decrease community variability.
The variance ratio has therefore been 
used [@frost1995species;@houlahan2007;@hallett2014;@ma2017climate] 
<!--DAN: Loreau said 2 of these don't actually use the VR, need to address-->
as an index of whether dynamics 
...
Whereas newer metrics such as $\phiLdM$ have some interpretive and other advantages over the 
...
[@Thibaut2013]. Timescale-specific alternatives and extensions of the original variance ratio approach have also been developed
[@vasseur2007; @brown2016;@zhao2020]. Although each of these alternative metrics improves upon the original variance ratio in 
at least some respects, we nevertheless construct the specific statistical
tools of this paper as extensions of the original variance ratio, for mathematical simplicity.
Also, this choice makes little difference, because the main concepts of this study can be applied as 
improvements to all existing frameworks of which we are aware.  We return to this topic in the Discussion.
...
To begin by giving an intuitive introduction to the main concepts of this study, we consider two simulated community dynamics datasets 
...
the $\mu_i$ and $v_{ij}$, $\phiLdM$ was also the same in our two scenarios, up to sampling variation
...
than in the second scenario (Fig. \ref{fig_pedag_cv2_skw}D, F; compare the probabilities on E and F listed to the left of the dashed line), 
whereas $\xtot(t)$ was more likely to explode to high values 
in the second scenario than in the first (compare the probabilities on E, F to the right of the dotted line).
...
abundances are more related to each other when the species are common, or rare (Fig. \ref{fig_pedag_cv2_skw}). 
...
(Fig. \ref{fig_pedag_cv2_skw}). Finally, ways in which species relationships
influence community variability should be studied using the new measures.
...
The goals of this study are as follows, paralleling the literature gaps identified above. (G1) We develop 
and apply an approach to studying relationships
between species that complements classic approaches relying on covariances. (G2) We develop and apply an
approach to studying community variability that complements the traditional 
coefficient of variation. Finally, (G3) we evaluate how species relationships influence community variability
using the new approaches, and using data from long-term studies from mixed-grass prairie in Hays, Kansas and 
...
Overall, our work establishes the beginnings of a more flexible paradigm for
conceptualizing synchronous and compensatory dynamics and their influence on community variability.
...
and $\phicv$. Thus, as in the example of Fig. \ref{fig_pedag_cv2_skw}, 
...
We also compare each scenario to a reference *comonotonic* scenario, for which 
...
and the panels below them are reproduced from panels C and D, respectively, for comparison).
...
quantity $\var(\xtot)$ gives the impression of a much more stable community.  
...
\noindent The skewness ratio $\phiS$ provides information about the influence of species relationships, of all kinds,
...
variability, in fact this true only for mitigating crashes of $\xtot$ to low values; species relationships 
...
that are readily interpretable as instability. Thus the classic and widely 
