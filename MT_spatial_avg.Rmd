...
<!--***DAN: Dan to write the corresponding section of the SI.-->
...
<!--***DAN: Add scenario labels to Fig. 1-->
...
<!--***DAN: Don't forget to create the SI section mentioned above-->
...
\ref{SI-tab_spinfo_hays} and \ref{SI-tab_spinfo_knz}. 
...
$\cor_l(x_i,x_j)-\cor_u(x_i,x_j)$, for all $i \neq j$, that were positive, representing stronger lower- than
upper-tail association between species; as well as the  number, $n_U$, of values 
that were negative, representing stronger upper- than lower-tail association between species.
We also computed the sum, $A_{\text{tot}}$, of $\cor_l(x_i,x_j)-\cor_u(x_i,x_j)$
...
negative if the reverse. We refer to $A_{\text{tot}}$ as the *total community tail association*
of positively associated species pairs. Positive association interpreted to mean a positive 
...
a hypothesis that for communities for which $n_L>n_U$ and $A_{\text{tot}}>0$, we should have 
...
First, on Fig. 5, please remove the red/blue shading behind the green-dotted squares (but only those squares - the non-green-dotted
squares should keep their shading, of course). It seems to me the shading on the green-dotted squares is not relevant to out main
arguments. We just did it because we could. And in order to do it, we had to flip one of the two variables. That adds complexity 
which we don't use for anything. So better to just show the green dot to indicate which species pairs were negatively associated,
but don't do anything else with those. The range of the colorbar at the bottom should be modified, if necessary, to reflect the range
of the values that are then displayed. Let me know if you see a reason I don't currently see why we should not make this change.
To me it seems like a simplification with no consequence, since we are not currently using the results for compensatory species for
anything, but perhaps I am missing something.

***Shya: ok, I fill those cells with green colors.

Second, please split Fig. 6 into 2 parts, the top part and bottom part. Please put the bottom part in the SI. Please make a version of the top part using powerpoint, so we can edit, and either send it to me or put it in the repo (OK to commit a powerpoint in this case). Replace the current Fig. 6 with a pdf export of this new version, which is the top part only. You can also add the pdf export to the repo. Don't worry about the captions. Also please make the 
changes to the top part of Fig. 6 (which will become the whole of Fig. 6) so that it looks like the drawing I have sent you, and so that phi_CV,ta is, instead of being indicated as being approximately 1, instead given to the same number of significant figures as everything else, using scientific notation if necessary. This applies to both locations where a value of phi_CV,ta is listed.

***Shya: Done.


Please compute the fraction of surrogate values on the current Fig. 6F that are to the *left* of the black dot. This is the 
...
Please compute the fraction of surrogate values on the current Fig. 6H that are to the *right* of the black dot. This is the 
...
each of two exemplary communities, we computed lower and upper tail 
...
cells (indicated by green cells), we measured the tail asymmetry by taking 
...
diagram but for Konza.\label{fig_result_box}}
\end{center}
\end{figure}
