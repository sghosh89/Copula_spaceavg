...
A commonly used stability-index is the 'variance ratio' (say, $\phi_{cv}$) that measures how  
community-variance changes compared to the consisting species if acting independently. 
Community with lower variance than the independent one (i.e., $\phi_{cv}<1$), is considered 
to be more stable due to overall compensatory dynamics whereas a community with higher variance than 
the independent one (i.e., $\phi_{cv}>1$), is considered to be less stable due to 
overall synchronous fluctuations. Therefore, the variance ratio ($\phi_{cv}$) considers 
if the species are negatively (compensation) or positively (synchronous) correlated 
among themselves within the community. An independent community means where species fluctuate 
randomly with zero correlation.
...
could incorporate thi "skewness changing" feature of the community. Fig 2(A-B): Box simple diagram.
...
\begin{figure}[!h]
\begin{center}
\includegraphics[width=13cm]{./Results/pedagog_figs/pedagog_complex_cv2_skw.pdf}\\
\includegraphics[width=16cm]{./Results/pedagog_figs/method_box_diagram.pdf}
