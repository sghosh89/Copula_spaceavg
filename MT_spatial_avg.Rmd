...
This is often referred to as *compensatory dynamics*, because decreases in species abundances are compensated
...
Total community biomass was left-skewed, frequently crashing to low values, for scenario 1; but was right skewed, frequently
...
Third, we ought to then study how species relationships
...
\noindent In this section we illustrate theoretical concepts for how relationships between
constituent species of a community, beyond just pairwise covariances/correlations of the species
time series, can influence the variability of an aggregate community property such as the 
total community biomass. The setup for the theory is as follows. As in the Introduction, let $x_i(t)$ be an abundance 
measure for species $i=1,\ldots,N$ at time $t=1,\ldots,T$. We use $N=20$ and $T=100000$ in the examples of this 
section. We assume the multivariate random variables $x(t)=(x_1(t),\ldots,x_N(t))$ are independent
...
classic approaches consider the covariances
$v_{ij}=\cov(x_i,x_j)$, and quantities based on them. We will instead consider, for each $i,j$, the whole 
joint distribution $(x_i,x_j)$, and some quantities based on it that are distinct from the $v_{ij}$.
To describe community variability, classic
approaches such as the variance ratio approach consider the variance of 
$\xtot=\sum_i x_i$, and quantities based on it (such as the 
squared coefficient of variation of $\xtot$, i.e. $\CVcomsq$). But this is only a summary of the 
distribution $\xtot$. We will instead consider the 
whole distribution of $\xtot$, and its skewness. 
We will present four scenarios (labeled 1-4 below) for which the marginal distributions $x_i$ and
the covariances $v_{ij}$ are the same
in all scenarios, as is the quantity $\var(\xtot)$ and the variance ratio $\phi_{\text{cv}}$. Thus 
our scenarios are indistinguishable from the standpoint of classic approaches. But our scenarios are,
in fact, ecologically distinct, as we will see, in that the total biomass $\xtot$ achieves much
...
Our scenarios are intentionally as simple as
possible, with complexities of population dynamics such as autocorrelation not included, so that the main
new ideas can be clearly described. But the ideas here apply to real communities, as we will see in 
subsequent sections of the paper. We will also compare our four scenario to a reference scenario, a 
so-called *comonotonic* scenario for which the $x_i$ are deterministically related via perfect positive relationships. 
This is a scenario of maximal covariance between species
and a maximal value of $\var(\xtot)$.

In scenario 1, which is a baseline scenario to which the other scenarios will be compared, 
...
and marginal histograms (side panels of Fig. \ref{fig_theory}A) help illustrate that this is a simple
multivariate normal distribution. The marginal distributions $x_1$ and $x_2$ will be the same in all scenarios.
...
low threshold is in Fig. \ref{fig_theory}D for a range of low thresholds. 

The other scenario for comparison is the comonotonic scenario, scenario C, for which we assume in this example that $x_i=x_1$ for all $i$. 
This is a specific form of comonotonicity. Comonotonicity between $x_1$ and $x_2$ is pictured in Fig.  \ref{fig_theory}E.
Because synchrony between the dynamics of individual species is perfect in this case, $\sd(\xtot)$ is much larger
than it was in scenario 1 (Fig.  \ref{fig_theory}F). The probabilities of $\xtot$ exceeding a high threshold or falling
below a low threshold are likewise larger (Fig.  \ref{fig_theory}G-H) than they were for scenario 1.

Scenarios 2-4 illustrate the ideas, previously little recognized within ecological research, 
of tail comonotonicity [@cheung2013] and tail associations [@Ghosh_extinction;@Ghosh_copula], and the consequences of tail
associations for the distribution of $\xtot$ and the probability of $\xtot$ passing a threshold. 
In scenario 2, each $x_i$ is again standard-normally distributed (Fig. \ref{fig_theory}I side panels),
but $(x_1,\ldots,x_N)$ is not a multivariate normal distribution. Instead, $(x_1,\ldots,x_N)$ was engineered so
that $x_i$ and $x_j$ are perfectly correlated when they exceed some threshold, but imperfectly correlated 
below the threshold (Fig. \ref{fig_theory}I; see SI section \ref{SI-TheoryMethods} for 
details). Correlation below the threshold was selected 
to make $\cov(x_i,x_j)$ equal to the same value that pertained in scenario 1, and so $\sd(\xtot)$ was also
the same, up to sampling variation, as it was in scenario 1 (Fig. \ref{fig_theory}J). (Recall that 
$\var(\xtot)=\sum_{i,j} \cov(x_i,x_j)$ and that the standard deviations is the square root of the variance.)
Comonotonicity in the upper tails produces right/positive skewness in $\xtot$ (Fig. \ref{fig_theory}J). The
skewness translates to elevated probabilities of $\xtot$ exceeding high thresholds (Fig. \ref{fig_theory}K),
compared to the baseline of scenario 1. In fact, the probability of exceeding sufficiently high thresholds is the
same as for scenario C. So, although scenarios 1 and 2 are indistinguishable to the classic variance ratio approach
and related approaches, the probability of exceeding high thresholds in scenario 2 is much elevated compared 
to scenario 1, and, in fact, is the same as scenario C and hence is as high as it can possibly be given the species marginal
distributions used in all our scenarios. The community
of scenario 2 is maximally unstable in the sense that its probability of exceeding certain high thresholds is maximal,
given fixed species marginal distributions, although it does not appear unstable to the classic variance ratio approach.

Scenario 3 is like scenario 2, but the comonotonicity is in the left tails instead of the right. 
Each $x_i$ is again standard-normally distributed (Fig. \ref{fig_theory}M side panels), but for this scenario
$(x_1,\ldots,x_N)$ was engineered so
that $x_i$ and $x_j$ are perfectly correlated when they go below some threshold, but imperfectly correlated 
above the threshold (Fig. \ref{fig_theory}M; see SI section \ref{SI-TheoryMethods} for details).
Correlation above the threshold was selected 
to again make $\cov(x_i,x_j)$ equal to the same value that pertained in scenario 1, and so $\sd(\xtot)$ was also
the same, up to sampling variation, as it was in scenario 1 (Fig. \ref{fig_theory}N).
Comonotonicity in the lower tails produces left/negative skewness in $\xtot$ (Fig. \ref{fig_theory}N). The
skewness translates to elevated probabilities of $\xtot$ falling below low thresholds (Fig. \ref{fig_theory}P),
compared to the baseline of scenario 1. The probability of falling below sufficiently low thresholds is the
same as for scenario C. Although scenarios 1-3 are indistinguishable to classic approaches, 
the probabilities of exceeding high thresholds in 2 or falling below low thresholds in 3 are as 
high as they can possibly be, given fixed species marginals. The communities of scenarios 2 and 3 
are maximally unstable in the right and left tails, respectively. 
Scenario 4 shows comonotonicity in both tails. Therefore probabilities of exceeding high thresholds or falling below
low thresholds are as high as they can be, even though scenario 4 again appears the same as 1-3 to classic approaches.
...
Additional details of the sites and initial processing efforts are in SI section \ref{SI-Data}.
...
and their effect on community variability because relationships between rare species are difficult to accurately
assess. We therefore categorized species as "common" (present in the quadrats considered for at last 35 years),
...
\ref{SI-tab_spinfo_hays} and \ref{SI-tab_spinfo_knz}. Henceforth the terminology "species" is understood
to include both true species the psudo-species which was formed by combining rare and intermediate species.

## Statistical methods

<!--Normalized rank plots and partial spearman-->
\noindent Fig. \ref{fig_pedag_cv2_skw} and the Theory section
suggest that to accomplish goal G1 of the Introduction, we will need statistical measures of tail associations between species, 
...
Given two bounds $0\leq b_l < b_u \leq 1$ ($l$ stands for "lower" and $u$ for "upper"), we define the lines $u_i + u_j=2b_l$ and $u_i+v_i=2b_u$,
...
\begin{equation}
\cor_{l_b,u_b}(x_i,x_j) = \frac{\sum (u_i(t)-\mean(u_i))(u_j(t)-\mean(u_j))}{(T-1)\sqrt{\var(u_i) \var(u_j)}},\label{PartialSpearmanEq}
\end{equation}
...
associations. Positive values indicate stronger lower-tail association, and negative values 
...
rank-based approaches such as Spearman correlations and the normalized rank plots and partial Spearman
correlations we introduce here. @Ghosh_copula, @Ghosh_extinction and @Ghosh_aphidplankton provide additional information on
why rank-based approaches and the partial Spearman correlation are appropriate for purposes such as ours. 
...
We computed $\cor_l-\cor_u$ for all positively associated species (positive Spearman correlation); negatively associated 
pairs were ignored. Fig. \ref{fig_pedag_cv2_skw} and the Theory section
suggest that the aggregate nature of tail associations of positively associated species may influence
the distribution of $\xtot$. Tail associations between positively associated species were 
measured, in aggregate for a community, by counting the number, $n_L$, of values
...
unique symmetric unbiased estimator of that quantity. As illustrated in Fig. \ref{fig_pedag_cv2_skw}, the skewness 
...
Fig. \ref{fig_pedag_cv2_skw}, either of these dynamical behaviors (i.e., either positive or negative
values of $\sk(\xtot)$) can happen independently of the values of $\var(\xtot)$ and $\CVcomsq$. 

Just as, in the classic approach, the variance ratio $\phi_{\text{cv}}$ is the quotient $\CVcomsq/\CVindsq$, we can 
analogously define a *skewness ratio* for our new framework, as follows. Let $\Scom=\sk(\xtot)$, and consider the value that 
...
We define the *skewness ratio* as $\phi_S=\Scom/\Sind$, so that $\Scom=\phi_S \Sind$. 
...
the skewness, $\Scom$, of $\xtot$ has greater magnitude, but the same sign, as what its value would be in 
the absence of species interactions, i.e., $\Sind$. So in such a case, and assuming for simplicity $\phi_{\text{cv}} \approx 1$
...
relationships reduce community instability. If $\phi_S \approx 0$,
...
exhibits occasional explosions to high values (respectively, crashes). 
Our skewness ratio should in no way be seen to supplant the classic variance ratio approach, but rather
complements it. Just as one understands more about a distribution by knowing both its variance and skewness, so
one can understand more about community dynamics by combining the variance ratio and skewness
ratio approaches. The values of $\phi_{\text{cv}}$ and $\phi_S$ together provide two complementary
pieces of information about how the distribution of $\xtot$ is influenced by species interactions (see Discussion).
...
on the skewness of $\xtot$. But goal G3 of the Introduction was to understand the influence of species tail associations,
...
the $\Scom$ was obtained by examining whether $\Scom$ fell sufficiently in the tails of the distribution 
of surrogate values to meet the desired confidence level. 

The quantity $\Snta$ was defined as the median of the values $\sk(\sum_i x_i^{(m)})$ for the surrogate datasets,
...
represents the factor by which asymmetric tail associations between species further influence the 
skewness ratio. So a value of $\phi_{S,\text{ta}}$ which differs from $1$ indicates that asymmetric tail associations have a role in
...
quantities and their interrelationships. However, because $\CVcomsq$ and $\phi_{\text{CV}}$ depend only 
on variances, covariances and means of species, which are preserved by our surrogates, $\CVntasq$ should approximately
equal $\CVcomsq$, and $\phi_{\text{CV},\text{ta}}$ should be approximately 1.

Having defined our methods, we can now frame hypotheses suggested by Fig. \ref{fig_pedag_cv2_skw} and the reasoning of the 
Theory section in terms of the new methods. Fig. \ref{fig_pedag_cv2_skw}A,C,E and 
Fig. \ref{fig_theory}M-P suggest that communities exhibiting a preponderance of lower-tail association 
should also have total community dynamics that are more left-skewed, or less right-skewed, than would have been the case
...
$\Scom<\Snta$, and therefore $\phi_{S,\text{ta}}<1$. 
Fig. \ref{fig_pedag_cv2_skw}B,D,F and 
Fig. \ref{fig_theory}I-L suggest that communities exhibiting a preponderance of upper-tail association 
should also have total community dynamics that are more right-skewed, or less left-skewed, than would have been the case
without these tail associations. Using the language of our new methods, this constitutes
a hypothesis that for communities for which $n_L<n_U$ and $\Atot<0$, we should have 
$\Scom>\Snta$, and therefore $\phi_{S,\text{ta}}>1$.
...
values. All analyses were done in R. Complete code for this project is at https://github.com/sghosh89/Copula_spaceavg. 
...
To address goal G1 of the Introduction, we computed $\cor_l-\cor_u$ (Methods) for all positively associated 
pairs of species within the Hays and
Konza datasets, and the community-aggregate indices $n_L$, $n_U$, and $\Atot$ (Fig. \ref{fig_CorlmCoru}),
as detailed in Methods. 
Although both datasets had pairs of species with predominantly lower-tail association and pairs of species
with predeominantly upper-tail association, $n_L$ was greater than $n_U$ for Hays and $n_L$ was less than $n_U$ for Konza: 
species pairs with predominanty lower-tail association were more common at Hays, and species with predominantly
upper-tail association were more common at Konza. The aggregate index $\Atot$ (Methods) was also positive for Hays
and negative for Konza (Fig. \ref{fig_CorlmCoru}). 

We earlier framed the hypotheses that communities exhibiting a preponderance of lower-tail association 
should also have $\xtot$ that is more left-skewed, or less right-skewed, than would have been the case
without species tail associations; and that communities exhibiting a preponderance of upper-tail association 
should have $\xtot$ that is more right-skewed, or less left-skewed, than would have been the case
without these tail associations (Methods). To address goals G1 and G2 of the Introduction, we tested
these hypotheses. The quantities of Fig. \ref{fig_method_box} were computed for our 
two datasets and are displayed in Fig. \ref{fig_result_box}.  
Because $n_L>n_U$ and $\Atot>0$ for Hays (Fig. \ref{fig_CorlmCoru}A), it was hypothesized 
that $\Scom$ should be less than $\Snta$, and 
this was confirmed (Fig. \ref{fig_result_box}B). The component of the skewness ratio  
due to tail association was, indeed, less than 1,
i.e., $\phi_{S,\text{ta}}=0.2757<1$. 
<!--***DAN: Shyamolina, please make the above number an autolink-->
Becuse $n_L<n_U$ and $\Atot<0$ for Konza (Fig. \ref{fig_CorlmCoru}B), it was hypothesized 
that $\Scom$ should be greater than 
$\Snta$, and this was confirmed (Fig. \ref{fig_result_box}D). For this dataset, $\phi_{S,\text{ta}}=2.7999>1$.
<!--***DAN: Shyamolina, please make the above number an autolink-->
These results were statistically significant: for Hays, $\Scom$ was less than a fraction $`r round(1-pval_skw_hays,4)`$
of the quantities $\sk(\sum_i x_i^{(m)})$ for surrogates $m=1,\ldots,M$ (Methods);
whereas for Konza, $\Scom$ was greater than a fraction $`r round(1-pval_skw_knz,3)`$ of the analogous surrogate 
values (but using surrogates of the Konza data; Fig. \ref{SI-fig_result_box}). Thus asymmetric 
tail associations between species can 
significantly modify the skewness of the total community property, $\xtot$, either raising it (upper-tail asociations)
it or lowering it (lower-tail associations).  

<!--***DAN: stopped here on 2020 06 23-->
