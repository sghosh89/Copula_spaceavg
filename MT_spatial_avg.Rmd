...
coexisting species is known to have a major impact on community stability, i.e., the relative constancy, 
through time, of aggregate properties such as total community abundance or biomass. 
...
measure of community stability which extends standard measures. We develop a “skewness 
ratio” which quantifies the extent to which relationships between species, and asymmetric tail associations in 
these relationships, modify community stability. The skewness ratio is analogous 
to, but complements, the classic variance ratio and related metrics based on covariances. 
Using two multi-decadal grassland datasets, we 
...
We begin by giving an intuitive introduction to how the main concepts of this study can extend previous approaches; 
...
and the covariances $v_{ij}$ will not detect the differences between our two scenarios, by construction. The correlation-based
...
values, the differences we have observed between these systems illuminate some of the potential 
applied possibilities of the ideas we explore. Whereas we emphasize that the data we used were from 
livestock-excluded plots, and hence applications to grazing are suggestive rather than direct, 
our overall results nevertheless show that details of the nature of species interactions can mitigate or
...
we have showed that tail associations between species can have important effects on extreme behavior of ecosystem 
functioning variables, and hence there is applied importance for any scenario in which extreme values would be 
a concern. See also below for discussion of how our methods may have applications in other fields.
...
improve these techniques. At least one paper in this special issue seems
...
a simple, univariate description. For instance,
@LenseInReview indicate that many childhood neurodevelopmental disorders are associated with deficits in 
...
Perhaps diagnosis of childhood neurodevelopmental disorders
may eventually be improved by appropriately formulating and applying
...
but many studies
of synchrony use univariate descriptions. 
It is plausible that improved statistics could lead to improved results
across many fields, as was the case in this study.
...
multifarious effects on community variability. 
...
empirical parallel to aspects of Fig. 1. Vertical ticks
