...
<!--***DAN: Add a citation at an appropriate place in the Intro to the PNAS paper that Max recently sent-->
...
threshold, then abundance fluctuations for the species may be more similar when the species are both abundant than
when they are rare. After 
...
species dynamics are partly synchronized in their fluctuations. 
<!--***DAN: This seems like a good place to add sentence including a citation to the recent PNAS paper that Max sent-->
Thus an important part of what determines 
...
Several extensions and improvements of the variance ratio approach have been proposed, but the original variance ration, $\phicv$, 
...
synchronous dynamics simultaneously occur on other timescales. Other improvements and alternatives to $\phicv$ 
...
by any existing extensions of which we are aware. 

To begin to illustrate the concepts we will explore in this 
study, and to start to provide an intuition on what has been omitted from earlier approaches, we consider two artificially
generated community dynamics datasets 
(Fig. \ref{fig_pedag_cv2_skw}A, B; data generated as in SI section \ref{SI-PedagogFigMethods}).
...
through time) were the same, up to sampling variation, in both scenarios. Likewise, data were 
...
[@loreau2008species], which itself depends only on the $v_{ij}$, was also the same, up to sampling variation, in the two scenarios 
...
$x_i$ and $x_j$ are related to each other in the tails of their distributions.
...
whole distribution of $\xtot$, including its skewness. 
...
than for scenario 1 (Fig.  \ref{fig_theory}F; the dashed black lines on that panel and the panels below it are 
reproduced from 
Fig. \ref{fig_theory}B, to facilitate comparisons). Probabilities of $\xtot$ exceeding a 
high disaster threshold or falling
below a low one are likewise larger (Fig.  \ref{fig_theory}G-H; dashed black lines on those panels
and the panels below them are again reproduced from panels C and D, respectively, for comparison).
...
scenario 1 (Fig. \ref{fig_theory}K; the dashed red lines on panels K and L are copied from panels G and H, 
respectively, to facilitate comparisons, likewise for panels below K and L). The probability of 
exceeding sufficiently high disaster thresholds is, in fact, the
...
Scenario 4 shows comonotonicity in both tails. Therefore probabilities of $\xtot$ exceeding high disaster thresholds or falling below
...
the probabilities of disaster are elevated, and are maximal given species marginal distributions, for some
of these scenarios and for some dister types (high or low disaster thresholds). 
...
necessary to generate skewness in $x_{\text{tot}}$; similar result pertain for essentially any case where
associations between species are asymmetric in the tails. 
...
Community data often include numerous rare species, e.g., species which were only detected a few times over the 
...
interspecific relationships, that mainly determine community stability. 
<!--***DAN: Cite the PNAS paper that Max sent recently, here-->
Common species are listed in Tables
...
represents the factor by which asymmetric tail associations between species further influence community skewness. 
...
species relationships on community skewness into the factors due to asymmetries of tail associations; and correlations,
...
we should have $\CVntasq \approx \CVcomsq$ and $\phicvta \approx 1$.
...
Although both the Hays and Konza datasets had pairs of positively associated species with more 
...
interactions, whereas explosions of $\xtot$ to high values are actually accentuated. 
...
In both scenarios, there were two groups of species (red and black lines on A, B).
...
\caption{Simple theoretical scenarios demonstrating how species interrelationships can influence a 
community aggregate
quantity in ways that are not detectable to classic approaches. The abbreviation ``sd'' stands 
for ``standard deviation'', ``sk'' stands for ``skewness'', and ``th'' stands
for ``threshold''. See the Theory section for other notation and 
interpretations, and SI section 
...
The partial Spearman correlation, $\cor_{b_l,b_u}(x_i,x_j)$, within a band defined by $b_l$ and $b_u$
